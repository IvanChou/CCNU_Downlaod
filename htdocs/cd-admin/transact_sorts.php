...
if (isset($_REQUEST['action'])) {
	switch ($_REQUEST['action']) {
		...
		if(!empty($term_name)){
			AddTerm($term_name,$term_rank);
		}else{
			echo "<script language='javascript'>alert('栏目名称为空！');location.href='sorts.php?title=sorts&list=view';</script>";
		}
        break;
		...
		if (!empty($tag_father)&&!empty($tag_name)){
			AddTag($tag_name,$tag_rank,$tag_father);
		}else{
			echo "<script language='javascript'>alert('二级栏目名称为空，添加失败！');location.href='add_tag.php?term_id=$tag_father';</script>";
		}
        break;
		...
		if(!empty($tag_id)){
			...
			echo "<script language='javascript'>alert('删除出错！');location.href='sorts.php?title=sorts&list=view';</script>";
		}
        break;
		...
		if(!empty($tag_id)&&!empty($tag_father)&&!empty($tag_name)){
			...
			echo "<script language='javascript'>alert('修改失败！');location.href='sorts.php?title=sorts&list=view';</script>";
		}
		break;
